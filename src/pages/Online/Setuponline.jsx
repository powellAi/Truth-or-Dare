...
export default function SetupOnline({ goTo, startGame, initialPlayers = [] }) {
  const theme = useTheme()
  const [step, setStep] = useState(initialPlayers.length >= 2 ? 2 : 1)
  const [players, setPlayers] = useState(
   initialPlayers.map(p => ({ name: p.displayName,  uid: p.uid, gender: "other", points: 0, skips: 5 }))
  )
  ...
  const [randomPickType, setRandomPickType] = useState(false)
  const [pickTimerSeconds, setPickTimerSeconds] = useState(10)
  const [askerPicksQuestion, setAskerPicksQuestion] = useState(false)
  ...
  if (step === 1) return (
    <div style={pageStyle}>
      <div style={maxWidth}>
        ...
        <p style={labelStyle}>Truth or Dare Selection</p>
        <button onClick={() => setRandomPickType(!randomPickType)} style={{
          padding: "14px 16px", borderRadius: "10px",
          border: randomPickType ? `2px solid ${theme.text}` : `1.5px solid ${theme.border}`,
          background: randomPickType ? theme.btnBg : theme.surface,
          color: randomPickType ? theme.btnText : theme.textMuted,
          fontWeight: "700", width: "100%", cursor: "pointer",
          marginBottom: "10px", fontSize: "0.9rem", textAlign: "left", transition: "all 0.2s",
        }}>
          {randomPickType ? "Random — app picks truth or dare automatically" : "Player Choice — answerer picks truth or dare"}
        </button>

        {!randomPickType && (
          <>
            <p style={{ ...labelStyle, marginTop: "16px" }}>Choice Time Limit</p>
            <div style={{ display: "flex", alignItems: "center", gap: "16px", marginBottom: "24px" }}>
              <button onClick={() => setPickTimerSeconds(Math.max(0, pickTimerSeconds - 5))}
                style={{ ...btnSecondary, width: "auto", padding: "10px 20px", marginTop: "0" }}>−</button>
              <span style={{ fontSize: "1.8rem", fontWeight: "900", color: theme.text, minWidth: "80px", textAlign: "center" }}>
                {pickTimerSeconds === 0 ? "Off" : `${pickTimerSeconds}s`}
              </span>
              <button onClick={() => setPickTimerSeconds(pickTimerSeconds + 5)}
                style={{ ...btnSecondary, width: "auto", padding: "10px 20px", marginTop: "0" }}>+</button>
            </div>
          </>
        )}
  ...
  if (step === 3) return (
    <div style={pageStyle}>
      <div style={maxWidth}>
        ...
        <p style={labelStyle}>Question Selection</p>
        <button onClick={() => setAskerPicksQuestion(!askerPicksQuestion)} style={{
          padding: "14px 16px", borderRadius: "10px",
          border: askerPicksQuestion ? `2px solid ${theme.text}` : `1.5px solid ${theme.border}`,
          background: askerPicksQuestion ? theme.btnBg : theme.surface,
          color: askerPicksQuestion ? theme.btnText : theme.textMuted,
          fontWeight: "700", width: "100%", cursor: "pointer",
          marginBottom: "24px", fontSize: "0.9rem", textAlign: "left", transition: "all 0.2s",
        }}>
          {askerPicksQuestion ? "On — asker picks from a list of questions" : "Off — question picked automatically"}
        </button>
